	...
	const login = async (magicEmail: string) => {
		...
		try {
			...
			if (error instanceof Error) {
				...
				console.log(error);
				state = new Error('something went wrong signing in with the magic link 😞');
			}
		}
	};
	onMount(async () => {
...
<svelte:head>
	<title>Confirm Login | Airsoftarmory</title>
</svelte:head>
	...
	{:else}
		<div class="grid grid-cols-12 gap-4">
			<div class="col-span-12 lg:col-span-5">
				<p class="mb-2">
					We know you just clicked a magic link, but maybe you’re logging in from a different device
					to where you requested the login?
				</p>
				<p class="mb-4">In any case, please fill in your email address and submit this form!</p>
			</div>
			<form
